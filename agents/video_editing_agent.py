...
class VideoEditingAgent(BaseAgent):
    ...
    def _initialize_tools(self) -> Dict[str, AgentTool]:
        ...
        return {
            'download_video': VideoEditingAgentTool("download_video", "Download videos from YouTube, Facebook, and other platforms", VideoDownloadTool()),
            'analyze_video': VideoEditingAgentTool("analyze_video", "Analyze video for duration, resolution, and basic content info", VideoAnalysisTool()),
            'trim_video': VideoEditingAgentTool("trim_video", "Trim video to specified start and end times", VideoTrimTool()),
            'add_watermark': VideoEditingAgentTool("add_watermark", "Add text or image watermark to video", VideoWatermarkTool()),
            'extract_audio': VideoEditingAgentTool("extract_audio", "Extract audio track from video file", AudioExtractionTool())
        }
...
class VideoDownloadTool(RobustTool):
    ...
    def _fallback_lower_quality(self, error: Exception, parameters: Dict[str, Any], execution_id: str) -> ToolResult:
...
class VideoWatermarkTool(RobustTool):
    ...
    def __init__(self):
        super().__init__(
            name="add_watermark",
            description="Add text or image watermark to video"
        )
...
class AudioExtractionTool(RobustTool):
    ...
    def _execute_core(self, parameters: Dict[str, Any], execution_id: str) -> Any:
        ...
        cmd = [
            ...
            '-vn',  # No video
            '-acodec', self._get_audio_codec(format_type),
